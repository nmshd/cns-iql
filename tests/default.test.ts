...
test("Check validity", () => {
    ...
    for (const e of table) {
        ...
        if (result.isValid !== e.isValid) {
            assert.fail(`IQL query "${e.iqlQuery}" is unexepectedly ${e.isValid ? "invalid" : "valid"}.`)
        }
    }
})
...
test("Check queries", () => {
    ...
    for (const e of table) {
        ...
        if (!_.isEqual([...matches].sort(), [...e.matches].sort())) {
            assert.fail(`'${e.iqlQuery}' matches records [${matches.toString()}], expected [${e.matches.toString()}].`)
        }
    }
})
